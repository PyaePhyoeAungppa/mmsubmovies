      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in-up">
        ...
        <div className="lg:col-span-1">
          <div className="glass-card overflow-hidden sticky top-24">
            {s.poster_url ? (
              <img src={s.poster_url} alt={s.title} className="w-full aspect-[2/3] object-cover" />
            ) : (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-3">
              <span className="badge badge-series">Series</span>
              <span className="text-gray-500 text-sm">{s.release_year}</span>
              {s.rating > 0 && (
                <div className="flex items-center gap-1">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="#d4a853" stroke="none">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                  <span className="text-[#d4a853] text-sm font-semibold">{s.rating}/10</span>
                </div>
              )}
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-4">{s.title}</h1>
          ...
          {/* Watch Button - always link to Telegram channel */}
          <a
            href={SERIES_TELEGRAM_LINK}
            target="_blank"
          ...
          {s.description && (
            <div className="glass-card p-6">
              ...
              <p className="text-gray-300 leading-relaxed whitespace-pre-line">{s.description}</p>
            </div>
          )}
