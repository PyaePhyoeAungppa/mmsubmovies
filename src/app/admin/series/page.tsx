...
import { Plus, Search, Edit2, Trash2, Tv, LayoutGrid, List, SlidersHorizontal } from 'lucide-react'
...
export default function AdminSeriesPage() {
  ...
  async function handleImportSeries(data: MovieFormData[]) {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [series, searchQuery, selectedGenres, minYear, maxYear, minRating, sortBy])
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-zinc-500 text-xs mt-0.5">Manage your collection ({series.length})</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/series/new">
            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/20 rounded-xl h-9 px-4 text-sm">
              <Plus className="w-4 h-4 mr-2" />
              Add Series
            </Button>
          </Link>
          <ExcelImport type="series" onImport={handleImportSeries} />
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
          <div className="relative max-w-sm w-full">
            ...
            <Input
              ...
              className="pl-9 bg-[#12121a] border-white/10 text-white rounded-xl h-10 text-sm focus-visible:ring-emerald-500/50"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center text-xs text-zinc-500">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
              ...
              <span className="font-medium">
                ...
                {series.length > 0 && filtered.length !== series.length
                  ? ` of ${series.length}`
                  : ` total`}
              </span>
            </div>
            ...
            <select
              ...
              className="h-9 rounded-lg bg-[#12121a] border border-white/10 text-xs text-zinc-300 px-3 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500/50"
            >
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-[11px] font-semibold transition-all ${
                  isSelected
                    ? 'bg-emerald-500 text-[#0a0a0f] shadow-md shadow-emerald-500/30'
      ...
      <div className="rounded-xl border border-white/10 bg-[#12121a] overflow-hidden mb-8">
        {loading ? (
          ...
          <div className="p-16 text-center text-zinc-500 flex flex-col items-center">
            ...
            <p className="text-lg font-medium">{searchQuery ? 'No series match your search.' : 'No series yet.'}</p>
            {!searchQuery && <p className="text-sm mt-1">Add your first series to get started!</p>}
          </div>
        ) : (
          <>
            {viewMode === 'table' ? (
                ...
                <Table>
                  <TableHeader className="bg-black/20 hover:bg-black/20 border-b border-white/10">
                    <TableRow className="border-none hover:bg-transparent">
                      <TableHead className="text-zinc-400 font-semibold h-12">Series</TableHead>
                      ...
                      >
                        <TableCell className="py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <span className="font-semibold text-zinc-100 group-hover:text-emerald-400 transition-colors truncate max-w-[250px]">
                              {s.title}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell text-zinc-400">{s.release_year}</TableCell>
                        ...
                        <TableCell className="hidden lg:table-cell text-center">
                          <span className="text-zinc-300 font-mono">{(s.views_count || 0).toLocaleString()}</span>
                        </TableCell>
                        <TableCell className="hidden lg:table-cell text-center">
                          <span className="text-zinc-300 font-mono">{(s.clicks_count || 0).toLocaleString()}</span>
                        </TableCell>
                      ...
                      <div className="flex-1 p-3 sm:p-4 flex flex-col justify-between">
                        <div>
                          ...
                          <p className="text-xs text-zinc-500 mb-2">
                            {s.release_year}{' '}
                            {s.genre && s.genre.length > 0 && `• ${s.genre.slice(0, 2).join(', ')}`}
                          </p>
                          <div className="flex items-center gap-2 text-xs">
                            ...
                            <span className="text-zinc-500 ml-2">{(s.views_count || 0).toLocaleString()} views</span>
                          </div>
                        </div>
